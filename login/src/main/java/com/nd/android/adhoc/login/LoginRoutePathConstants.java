package com.nd.android.adhoc.login;

public class LoginRoutePathConstants {

    public static final String PATH_LOGIN_ROOT = "/login/";

    public static final String PATH_LOGIN_INIT = PATH_LOGIN_ROOT + "init";

    public static final String PATH_LOGIN_LOGIN = PATH_LOGIN_ROOT + "login";

    public static final String PATH_USER_INFO = PATH_LOGIN_ROOT + "userInfo";
}
